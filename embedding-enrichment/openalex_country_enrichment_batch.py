#!/usr/bin/env python3
"""
OpenAlex Country Enrichment Service (BATCH VERSION)
Uses batch queries to process up to 50 DOIs per request
10x faster than individual queries!

Expected runtime: ~42-84 minutes (vs 7 hours for individual queries)
"""

import requests
import time
import sys
import os
import logging
from typing import Optional, Tuple, Dict, Any, List
from urllib.parse import quote

sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'doctrove-api'))
from db import create_connection_factory

# Configure logging
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(levelname)s - %(message)s'
)
logger = logging.getLogger(__name__)

# ============================================================================
# CONFIGURATION
# ============================================================================

OPENALEX_EMAIL = "your-email@example.com"  # TODO: Update this!
BATCH_SIZE = 50  # OpenAlex allows up to 50 DOIs per request
RATE_LIMIT = 0.1  # 10 requests/second (polite pool)
DB_BATCH_SIZE = 1000  # Number of papers to fetch from DB at once

# ============================================================================
# COUNTRY CODE MAPPING (from old openalex_country_enrichment.py)
# ============================================================================

COUNTRY_CODE_TO_NAME = {
    'US': 'United States',
    'CN': 'China',
    'GB': 'United Kingdom',
    'DE': 'Germany',
    'FR': 'France',
    'CA': 'Canada',
    'AU': 'Australia',
    'JP': 'Japan',
    'IN': 'India',
    'BR': 'Brazil',
    'IT': 'Italy',
    'ES': 'Spain',
    'NL': 'Netherlands',
    'SE': 'Sweden',
    'CH': 'Switzerland',
    'KR': 'South Korea',
    'RU': 'Russia',
    'SG': 'Singapore',
    'IL': 'Israel',
    'NO': 'Norway',
    'DK': 'Denmark',
    'FI': 'Finland',
    'BE': 'Belgium',
    'AT': 'Austria',
    'IE': 'Ireland',
    'NZ': 'New Zealand',
    'PL': 'Poland',
    'PT': 'Portugal',
    'GR': 'Greece',
    'CZ': 'Czech Republic',
    'HU': 'Hungary',
    'TR': 'Turkey',
    'MX': 'Mexico',
    'AR': 'Argentina',
    'CL': 'Chile',
    'ZA': 'South Africa',
    # Add more as needed...
}

def country_code_to_uschina(country_code: str) -> str:
    """Convert country code to uschina category"""
    if country_code == 'US':
        return 'United States'
    elif country_code == 'CN':
        return 'China'
    elif country_code in COUNTRY_CODE_TO_NAME:
        return 'Other'
    else:
        return 'Unknown'

def country_code_to_name(country_code: str) -> str:
    """Convert country code to full name"""
    return COUNTRY_CODE_TO_NAME.get(country_code, country_code)

# ============================================================================
# OPENALEX BATCH API
# ============================================================================

def query_openalex_batch(dois: List[str], email: str = OPENALEX_EMAIL, use_proxy: bool = False) -> Dict[str, Dict[str, Any]]:
    """
    Query OpenAlex API for multiple DOIs in a single batch request.
    
    Args:
        dois: List of DOIs (up to 50)
        email: Email for polite pool (10 req/sec)
        use_proxy: Use SOCKS proxy (for AWS → local machine routing)
    
    Returns:
        Dictionary mapping DOI to OpenAlex data
    """
    if not dois or len(dois) == 0:
        return {}
    
    if len(dois) > 50:
        logger.warning(f"Batch size {len(dois)} exceeds limit of 50, truncating")
        dois = dois[:50]
    
    # Clean DOIs (remove https://doi.org/ prefix if present)
    clean_dois = [doi.strip().replace('https://doi.org/', '') for doi in dois]
    
    # Build pipe-separated DOI list and URL encode
    doi_filter = "|".join(clean_dois)
    doi_filter_encoded = quote(doi_filter, safe='|')  # Encode slashes but keep pipes
    
    # Construct API URL
    url = (
        f"https://api.openalex.org/works?"
        f"filter=doi:{doi_filter_encoded}"
        f"&per-page=50"
        f"&mailto={email}"
    )
    
    # Setup proxy if requested
    proxies = None
    if use_proxy:
        # Use SOCKS proxy (requires SSH tunnel: ssh -D 9050 user@local-machine)
        socks_proxy = os.environ.get('SOCKS_PROXY', 'socks5://localhost:9050')
        proxies = {
            'http': socks_proxy,
            'https': socks_proxy
        }
        logger.debug(f"Using SOCKS proxy: {socks_proxy}")
    
    try:
        response = requests.get(url, timeout=30, proxies=proxies)
        
        if response.status_code == 200:
            data = response.json()
            results = data.get('results', [])
            
            # Map results by DOI for easy lookup
            doi_map = {}
            for work in results:
                work_doi = work.get('doi')
                if work_doi:
                    # Normalize DOI for matching
                    work_doi_clean = work_doi.replace('https://doi.org/', '')
                    doi_map[work_doi_clean] = work
            
            logger.debug(f"Batch query returned {len(doi_map)} results for {len(dois)} DOIs")
            return doi_map
            
        elif response.status_code == 429:
            logger.warning("Rate limit exceeded, sleeping 60s")
            time.sleep(60)
            return query_openalex_batch(dois, email)  # Retry
            
        else:
            logger.error(f"OpenAlex API error: {response.status_code}")
            return {}
            
    except Exception as e:
        logger.error(f"Exception querying OpenAlex batch: {e}")
        return {}

# ============================================================================
# DATA EXTRACTION
# ============================================================================

def extract_institution_and_country(openalex_data: Dict[str, Any]) -> Optional[Tuple[str, str, str, str]]:
    """
    Extract institution and country from OpenAlex response.
    
    Returns:
        (institution_name, country_code, country_name, uschina) or None
    """
    authorships = openalex_data.get('authorships', [])
    if not authorships:
        return None
    
    # Get first author
    first_author = authorships[0]
    
    # Try to get institution
    institutions = first_author.get('institutions', [])
    if institutions:
        institution = institutions[0]
        institution_name = institution.get('display_name', 'Unknown')
        country_code = institution.get('country_code')
        
        if country_code:
            country_name = country_code_to_name(country_code)
            uschina = country_code_to_uschina(country_code)
            return (institution_name, country_code, country_name, uschina)
    
    # Fallback: try countries field directly
    countries = first_author.get('countries', [])
    if countries:
        country_code = countries[0]
        country_name = country_code_to_name(country_code)
        uschina = country_code_to_uschina(country_code)
        return ('Unknown', country_code, country_name, uschina)
    
    return None

# ============================================================================
# DATABASE OPERATIONS
# ============================================================================

def get_papers_needing_enrichment(conn_factory, limit: Optional[int] = None) -> List[Tuple[str, str]]:
    """
    Get arXiv papers with DOIs that haven't been enriched yet.
    
    Returns:
        List of (paper_id, doi) tuples
    """
    with conn_factory() as conn:
        with conn.cursor() as cur:
            query = """
                SELECT dp.doctrove_paper_id, am.arxiv_doi
                FROM doctrove_papers dp
                JOIN arxiv_metadata am ON dp.doctrove_paper_id = am.doctrove_paper_id
                LEFT JOIN enrichment_country ec ON dp.doctrove_paper_id = ec.doctrove_paper_id
                WHERE dp.doctrove_source = 'arxiv'
                AND am.arxiv_doi IS NOT NULL
                AND ec.doctrove_paper_id IS NULL
                ORDER BY dp.doctrove_paper_id
            """
            
            if limit:
                query += f" LIMIT {limit}"
            
            cur.execute(query)
            return cur.fetchall()

def save_enrichment_results(conn_factory, results: List[Dict[str, Any]]) -> int:
    """
    Save enrichment results to database.
    
    Args:
        results: List of enrichment result dictionaries
    
    Returns:
        Number of successful inserts
    """
    if not results:
        return 0
    
    success_count = 0
    
    with conn_factory() as conn:
        with conn.cursor() as cur:
            for result in results:
                try:
                    cur.execute("""
                        INSERT INTO enrichment_country (
                            doctrove_paper_id,
                            institution_name,
                            institution_country_code,
                            country_name,
                            country_uschina,
                            enrichment_method,
                            enrichment_confidence,
                            enrichment_source
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        ON CONFLICT (doctrove_paper_id) DO NOTHING
                    """, (
                        result['paper_id'],
                        result['institution_name'],
                        result['country_code'],
                        result['country_name'],
                        result['uschina'],
                        'openalex_api',
                        'high',
                        'OpenAlex API'
                    ))
                    success_count += 1
                except Exception as e:
                    logger.error(f"Error saving paper {result['paper_id']}: {e}")
                    continue
        
        conn.commit()
    
    return success_count

# ============================================================================
# MAIN PROCESSING
# ============================================================================

def process_arxiv_papers_batch(
    limit: Optional[int] = None,
    email: str = OPENALEX_EMAIL,
    use_proxy: bool = False
):
    """
    Process arXiv papers with DOIs using batch OpenAlex API queries.
    
    Args:
        limit: Maximum number of papers to process (for testing)
        email: Email for OpenAlex polite pool
        use_proxy: Route traffic through SOCKS proxy (for AWS → local routing)
    """
    conn_factory = create_connection_factory()
    
    # Get papers to process
    logger.info("Fetching papers needing enrichment...")
    papers = get_papers_needing_enrichment(conn_factory, limit)
    total = len(papers)
    
    if total == 0:
        logger.info("No papers need enrichment")
        return
    
    logger.info(f"Found {total:,} papers to process")
    
    # Statistics
    processed = 0
    matched = 0
    not_found = 0
    
    # Process in batches of 50 DOIs
    for i in range(0, total, BATCH_SIZE):
        batch = papers[i:i + BATCH_SIZE]
        batch_num = (i // BATCH_SIZE) + 1
        total_batches = (total + BATCH_SIZE - 1) // BATCH_SIZE
        
        # Extract DOIs and paper IDs
        paper_ids = [p[0] for p in batch]
        dois = [p[1] for p in batch]
        
        logger.info(f"Processing batch {batch_num}/{total_batches} ({len(batch)} papers)")
        
        # Query OpenAlex in batch
        openalex_results = query_openalex_batch(dois, email, use_proxy)
        
        # Process results
        enrichment_results = []
        for paper_id, doi in zip(paper_ids, dois):
            doi_clean = doi.strip().replace('https://doi.org/', '')
            
            if doi_clean in openalex_results:
                # Extract data
                openalex_data = openalex_results[doi_clean]
                extraction = extract_institution_and_country(openalex_data)
                
                if extraction:
                    institution_name, country_code, country_name, uschina = extraction
                    enrichment_results.append({
                        'paper_id': paper_id,
                        'institution_name': institution_name,
                        'country_code': country_code,
                        'country_name': country_name,
                        'uschina': uschina
                    })
                    matched += 1
                else:
                    not_found += 1
            else:
                not_found += 1
        
        # Save batch results to database
        if enrichment_results:
            saved = save_enrichment_results(conn_factory, enrichment_results)
            logger.info(f"  Saved {saved} enrichments")
        
        processed += len(batch)
        
        # Progress update
        if batch_num % 10 == 0 or batch_num == total_batches:
            logger.info(f"Progress: {processed:,}/{total:,} ({100*processed/total:.1f}%) - "
                       f"Matched: {matched:,}, Not found: {not_found:,}")
        
        # Rate limiting
        time.sleep(RATE_LIMIT)
    
    # Final statistics
    logger.info(f"\n{'='*80}")
    logger.info(f"Processing Complete!")
    logger.info(f"Total processed: {processed:,}")
    logger.info(f"Successfully matched: {matched:,} ({100*matched/processed:.1f}%)")
    logger.info(f"Not found in OpenAlex: {not_found:,} ({100*not_found/processed:.1f}%)")
    logger.info(f"{'='*80}")

# ============================================================================
# COMMAND LINE
# ============================================================================

def main():
    import argparse
    
    parser = argparse.ArgumentParser(
        description='OpenAlex Country Enrichment (Batch Version)'
    )
    parser.add_argument(
        '--limit',
        type=int,
        default=None,
        help='Limit number of papers to process (for testing)'
    )
    parser.add_argument(
        '--email',
        type=str,
        default=OPENALEX_EMAIL,
        help='Email for OpenAlex polite pool (10 req/sec)'
    )
    parser.add_argument(
        '--test',
        action='store_true',
        help='Test mode: process only 100 papers'
    )
    parser.add_argument(
        '--use-proxy',
        action='store_true',
        help='Use SOCKS proxy (set SOCKS_PROXY env var, default: socks5://localhost:9050)'
    )
    
    args = parser.parse_args()
    
    # Validate email
    if args.email == "your-email@example.com":
        logger.error("Please update OPENALEX_EMAIL in the script or use --email flag")
        logger.error("Including email gives you 10 req/sec (vs 1 req/sec)")
        return
    
    # Test mode
    limit = 100 if args.test else args.limit
    
    if limit:
        logger.info(f"Test mode: processing {limit} papers")
    
    # Check proxy setup
    if args.use_proxy:
        logger.info(f"Proxy enabled: {os.environ.get('SOCKS_PROXY', 'socks5://localhost:9050')}")
        logger.info("Make sure SSH tunnel is running on local machine:")
        logger.info("  ssh -D 9050 -C -N user@local-machine")
        
        # Test if PySocks is installed
        try:
            import socks
            logger.info("✓ PySocks is installed")
        except ImportError:
            logger.error("✗ PySocks not installed. Install with: pip install PySocks")
            return
    
    # Run processing
    process_arxiv_papers_batch(
        limit=limit,
        email=args.email,
        use_proxy=args.use_proxy
    )

if __name__ == "__main__":
    main()

