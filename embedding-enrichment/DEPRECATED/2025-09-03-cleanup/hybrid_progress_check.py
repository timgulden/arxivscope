#!/usr/bin/env python3
"""
Hybrid Embedding Progress Check

Uses fast database functions with proper math for rate calculations.
"""
    ...
    try:
        with psycopg2.connect(
            ...
            with conn.cursor() as cur:
                # Use the fast database functions
                ...
                with_full = total_papers - papers_needing_full
                with_2d = total_papers - papers_needing_2d
...
def main():
    """Main function."""
    print("🔍 Hybrid Embedding Progress Check")
    print("=" * 40)
    
    # Get current counts
    current = get_hybrid_counts()
    if not current:
    ...
    with open("embedding_progress_report.txt", "w") as f:
        f.write(f"Hybrid Embedding Progress Report - {current['timestamp']}\n")
        f.write("=" * 60 + "\n\n")
