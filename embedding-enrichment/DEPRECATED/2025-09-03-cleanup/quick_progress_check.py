#!/usr/bin/env python3
"""
Quick Embedding Progress Check

Fast progress monitoring using database functions and optimized queries.
"""
...
def get_quick_counts():
    """Get counts using fast database functions."""
    try:
        with psycopg2.connect(
            ...
            with conn.cursor() as cur:
                # Use database functions for fast counting
                cur.execute("SELECT get_papers_needing_embeddings_count();")
                papers_needing_full = cur.fetchone()[0]
                
                cur.execute("SELECT get_papers_needing_2d_embeddings_count();")
                papers_needing_2d = cur.fetchone()[0]
                
                # Get total from a fast estimate (using table stats)
                cur.execute("SELECT reltuples::bigint FROM pg_class WHERE relname = 'doctrove_papers';")
                total_estimate = cur.fetchone()[0]
                
                # Calculate current counts
                with_full = total_estimate - papers_needing_full
                with_2d = total_estimate - papers_needing_2d
                ...
                return {
                    ...
                    'papers_needing_full': papers_needing_full,
                    'papers_needing_2d': papers_needing_2d
                }
...
def main():
    """Main function."""
    print("🔍 Quick Embedding Progress Check")
    print("=" * 40)
    
    # Get current counts
    current = get_quick_counts()
    if not current:
