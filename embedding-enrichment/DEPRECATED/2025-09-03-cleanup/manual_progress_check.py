#!/usr/bin/env python3
"""
Manual Embedding Progress Check

Uses known numbers and proper math for rate calculations.
"""
...
def get_manual_counts():
    """Get counts using known numbers."""
    # These are the actual numbers from our database query
    total_papers = 17870457
    with_full = 4570748  # From our earlier query
    with_2d = 4197003    # From our earlier query
...
def main():
    """Main function."""
    print("🔍 Manual Embedding Progress Check")
    print("=" * 40)
    
    # Get current counts
    current = get_manual_counts()
    ...
    with open("embedding_progress_report.txt", "w") as f:
        f.write(f"Manual Embedding Progress Report - {current['timestamp']}\n")
        f.write("=" * 60 + "\n\n")
