"""
Runner for Institution-Cached OpenAlex Country Enrichment Service
Shows dramatic reduction in LLM calls by processing unique institutions only.
"""
...
def run_institution_cached_enrichment(batch_size: int = 1000, limit: Optional[int] = None, use_real_llm: bool = True):
    """
    Run institution-cached OpenAlex country enrichment.
    ...
    if use_real_llm:
        ...
        print("Using real Azure OpenAI service with institution caching")
    else:
    ...
    for i in range(0, len(papers), batch_size):
        ...
        # Process batch with institution caching
        results = enrichment.process_papers(batch)
        
        # Count LLM calls (this would need to be tracked in the enrichment service)
        # For now, we'll estimate based on the number of unique institutions
        if results:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run Institution-Cached OpenAlex Country Enrichment')
    ...
    parser.add_argument('--large-test', action='store_true',
                       help='Run a large test with 5000 papers')
    
    args = parser.parse_args()
    
    if args.small_test:
        print("Running small test with 100 papers...")
        run_institution_cached_enrichment(batch_size=100, limit=100, use_real_llm=not args.test)
    elif args.medium_test:
        print("Running medium test with 1000 papers...")
        run_institution_cached_enrichment(batch_size=500, limit=1000, use_real_llm=not args.test)
    elif args.large_test:
        print("Running large test with 5000 papers...")
        run_institution_cached_enrichment(batch_size=1000, limit=5000, use_real_llm=not args.test)
    else:
        print(f"Running institution-cached enrichment with batch_size={args.batch_size}, limit={args.limit}, test={args.test}")
        run_institution_cached_enrichment(batch_size=args.batch_size, limit=args.limit, use_real_llm=not args.test)
