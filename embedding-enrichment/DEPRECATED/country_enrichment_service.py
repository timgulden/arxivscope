...
def create_table_if_needed(connection_factory) -> None:
    """Create the openalex_enrichment_country table if it doesn't exist."""
    with connection_factory() as conn:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info("Table openalex_enrichment_country created/verified successfully")
...
def get_papers_needing_country_enrichment(connection_factory, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    query = """
        SELECT dp.doctrove_paper_id, dp.doctrove_title, dp.doctrove_abstract, 
               dp.doctrove_source, dp.doctrove_authors
        ...
        with conn.cursor() as cur:
            ...
            for row in cur.fetchall():
                paper = {
                    ...
                    'doctrove_source': row[3],
                    'doctrove_authors': row[4]
                }
                papers.append(paper)
            return papers
...
def process_country_enrichment_for_papers(papers: List[Dict[str, Any]], connection_factory) -> List[Dict[str, Any]]:
    ...
    from openalex_country_enrichment import create_openalex_country_enrichment
    
    try:
        # Create enrichment processor
        enrichment_processor = create_openalex_country_enrichment()
        
        # Process papers
        enrichment_results = enrichment_processor.process_papers(papers)
...
def mark_country_enrichment_failed(connection_factory, paper_id: str):
        ...
        with conn.cursor() as cur:
            for result in enrichment_results:
                try:
                    ...
                    """, (
                        result['paper_id'], result['openalex_country_country'], result['openalex_country_uschina']
                    ))
                    updated_count += 1
                    ...
                    # Mark as failed
                    mark_country_enrichment_failed(connection_factory, result.paper_id)
                    conn.rollback()
                    continue
...
def process_country_enrichment(batch_size: int = BATCH_SIZE, limit: Optional[int] = None) -> Dict[str, Any]:
    ...
    for i in range(0, total_papers, batch_size):
        ...
        try:
            # Process batch
            enrichment_results = process_country_enrichment_for_papers(batch, connection_factory)
            ...
            if enrichment_results:
                ...
                # Papers without results get marked as Unknown (not failed)
                papers_without_results = len(batch) - updated_count
                if papers_without_results > 0:
                    for paper in batch:
                        if not any(r['paper_id'] == paper['doctrove_paper_id'] for r in enrichment_results):
                            mark_country_enrichment_failed(connection_factory, paper['doctrove_paper_id'])
                    total_failed += papers_without_results
            else:
                # No enrichment results means no country data available - mark as Unknown
                for paper in batch:
                    mark_country_enrichment_failed(connection_factory, paper['doctrove_paper_id'])
...
def main():
    ...
    if args.status:
        # Show status
        # Ensure the table exists before checking status
        create_table_if_needed(connection_factory)
