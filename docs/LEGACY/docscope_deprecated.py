            ...
            "checks": [
                {
                    ...
                    "timestamp": time.strftime('%Y-%m-%d %H:%M:%S'),
                    "details": {"host": "0.0.0.0", "port": 8050},
                    "duration_ms": None
                }
            ],
            "metadata": {
...
def fetch_papers_from_api(limit: int = 10000, bbox: Optional[str] = None, 
    ...
    try:
        ...
        params = {
            'limit': limit,
            'fields': 'doctrove_paper_id,doctrove_title,doctrove_abstract,doctrove_source,doctrove_primary_date,doctrove_embedding_2d,country2'
        }
        ...
        if 'Country of Publication' not in df.columns:
            ...
            # Fill null/empty values with 'RAND' and rename 'Unknown' to 'RAND'
            df['Country of Publication'] = df['Country of Publication'].fillna('RAND')
            df.loc[df['Country of Publication'] == '', 'Country of Publication'] = 'RAND'
            df.loc[df['Country of Publication'] == 'Unknown', 'Country of Publication'] = 'RAND'
...
country_color_map = {
    ...
    'Unknown': '#9370DB',  # Purple for RAND papers
}

# Default color for other countries
default_color = '#9370DB'  # Purple for RAND papers
...
def load_initial_data() -> pd.DataFrame:
    ...
    df = fetch_papers_from_api(limit=TARGET_RECORDS_PER_VIEW)
    return df

# Create initial figure with dark background immediately
fig = go.Figure()
                        ...
                        html.Div([
                            ...
                            html.Div([
                                dcc.Checklist(
                                    ...
                                    options=[{'label': 'Show Regions', 'value': 'show'}],
                                    value=['show'],
...
@app.callback(
    ...
    [Input('graph-3', 'relayoutData'),
     Input('max-dots', 'value')],
    prevent_initial_call=False
)
def unified_data_store_callback(relayoutData, max_dots):
    global last_api_call_time, last_zoom_state
    
    # Use max_dots value or default to TARGET_RECORDS_PER_VIEW
    limit = max_dots if max_dots is not None else TARGET_RECORDS_PER_VIEW
    ...
    if relayoutData and 'autosize' in relayoutData:
        ...
        fetched_df = fetch_papers_for_view(limit=limit)
    ...
    print(f"Zoom callback: Fetching data for bbox {bbox} with limit {limit}")
    last_api_call_time = current_time
    fetched_df = fetch_papers_for_view(bbox=bbox, limit=limit)
    if fetched_df.empty:
...
def update_filters(data_store):
    ...
    df = pd.DataFrame(data_store)
    all_countries = sorted(df['Country of Publication'].unique())
    
    # Sort countries in the desired order: United States, China, Rest of the World, RAND
    desired_order = ['United States', 'China', 'Rest of the World', 'Unknown']
    countries = []
    
    # Add countries in desired order if they exist
    for country in desired_order:
        if country in all_countries:
            countries.append(country)
    
    # Add any remaining countries that weren't in the desired order
    for country in all_countries:
        if country not in countries:
            countries.append(country)
    
    # Create filter options with colors and better grouping
    filter_options = []
    for cat in countries:
        color = get_country_color(cat)
        
        # Create better labels for grouping
        if cat in ['United States', 'China', 'Rest of the World']:
            # Add "(ArXiv)" to recent papers
            display_name = f"{cat} (ArXiv)"
        elif cat == 'Unknown':
            # Show RAND papers as "All RAND Publications"
            display_name = "All RAND Publications"
        else:
            display_name = cat
...
def overlay_clusters(n_clicks, data_store, num_clusters, relayoutData, selected_countries):
    ...
    if not selected_countries:
        # If no countries selected, use all data
        filtered_df = df
        print("Cluster callback: No countries selected, using all data")
    else:
        filtered_df = df[df['Country of Publication'].isin(selected_countries)]
        print(f"Cluster callback: Filtered to {len(filtered_df)} papers from selected countries")
    ...
    if len(visible_df) >= num_clusters:
        ...
        for i, region in enumerate(voronoi_regions):
            ...
            if len(poly_points) > 0:
                first_point = poly_points[0]
                last_point = poly_points[-1]
                is_closed = (first_point[0] == last_point[0] and first_point[1] == last_point[1])
                
                # If not closed, add the first point to the end to close it
                if not is_closed and len(poly_points) > 2:
...
def update_dots(data_store, selected_countries, cluster_overlay, clickData, clear_trigger, show_region_labels, current_figure):
    ...
    # Create figure with dark background immediately
    ...
    show_regions = 'show' in show_region_labels if show_region_labels else False
    
    if cluster_overlay and 'polygons' in cluster_overlay and show_regions:
        for i, poly in enumerate(cluster_overlay['polygons']):
    ...
    if cluster_overlay and 'annotations' in cluster_overlay:
        ...
        for ann in cluster_overlay['annotations']:
            annotations.append(dict(
                ...
                borderwidth=0,
                opacity=1 if show_regions else 0
            ))
        fig.update_layout(annotations=annotations)
    
    # Check if this is the initial data load (no meaningful data in current figure)
    is_initial_load = True
    if current_figure and 'data' in current_figure and current_figure['data']:
        # Check if current figure has actual data points
        for trace in current_figure['data']:
            if 'x' in trace and 'y' in trace and len(trace['x']) > 0:
                is_initial_load = False
                break
    
    # Only preserve zoom if it's not the initial load and there's a meaningful zoom
    if not is_initial_load and current_figure and 'layout' in current_figure:
        ...
        # Only preserve the view if it's a meaningful zoom
        if (x_range and y_range and len(x_range) == 2 and len(y_range) == 2 and
            abs(x_range[1] - x_range[0]) > 0.1 and abs(y_range[1] - y_range[0]) > 0.1):
            fig.update_layout(xaxis_range=x_range, yaxis_range=y_range)
    
    # For initial load, let Plotly auto-fit the data (default behavior)
    
    # Update only the interactive settings, background is already set
    fig.update_layout(
        dragmode='pan',
        hovermode='closest',
        legend_title_text=' ',
    ...
    try:
        ...
        if "customdata" in point and point["customdata"] is not None:
            ...
            if isinstance(authors, list):
                ...
                authors_text = str(authors) if authors else 'No authors available'
            ...
            # Format DOI as clickable link if available
            doi_element = None
            if doi:
                ...
                doi_element = html.A(
                    ...
                    href=doi_url,
                    target="_blank",
                    style={'color': '#87CEEB', 'text-decoration': 'underline', 'margin-top': '5px'}
                )
            
            # Parse and format links if available
            links_elements = []
            if links:
                import json
                try:
                    links_list = json.loads(links) if isinstance(links, str) else links
                    for link in links_list:
                        if isinstance(link, dict):
                            ...
                            if rel == 'alternate' and ltype == 'text/html':
                                links_elements.append(html.A('arXiv Abstract', href=href, target='_blank', 
                                                           style={'color': '#87CEEB', 'margin-right': '15px', 'text-decoration': 'underline'}))
                            elif ltype == 'application/pdf':
                                links_elements.append(html.A('PDF', href=href, target='_blank', 
                                                           style={'color': '#87CEEB', 'margin-right': '15px', 'text-decoration': 'underline'}))
                except Exception as e:
                    links_elements.append(html.P(f"[Error parsing links: {e}]", style={'color': '#FFD700'}))
            ...
            children = html.Div(
                ...
                children=[
                    ...
                    html.P(f"Authors: {authors_text}", style={'color': '#ffffff', 'margin-top': '5px', 'font-style': 'italic'}),
                    html.P(f"Date: {formatted_date}", style={'color': '#ffffff', 'margin-top': '5px'}),
                    html.P(f"Source: {category}", style={'color': '#ffffff', 'margin-top': '5px'}),
                    doi_element,
                    html.Div(links_elements, style={'margin-top': '5px'}) if links_elements else None,
...
def generate_simple_voronoi_boundaries(df: pd.DataFrame, num_points: int = 50) -> dict:
    """
    Generate simple Voronoi boundaries for visualization without clustering.
    Uses a subset of points to create boundaries that help visualize the data structure.
    """
    if len(df) < 3:
        return {'polygons': [], 'annotations': []}
    
    # Sample points for Voronoi generation (don't use too many for performance)
    sample_size = min(num_points, len(df))
    sample_df = df.sample(n=sample_size, random_state=42)
    
    # Get coordinates
    points = sample_df[['x', 'y']].values
    ...
    try:
        ...
        regions = voronoi_finite_polygons_2d(vor, bbox)
        ...
        return {'polygons': polygons, 'annotations': []}
        
    except Exception as e:
        print(f"Error generating Voronoi boundaries: {e}")
        return {'polygons': [], 'annotations': []}
