#!/usr/bin/env python3
"""
Update RAND publication source_ids from hash-based to MARC 001 control numbers.
...
def update_source_ids():
    """Update RAND publication source_ids with MARC 001 control numbers"""
    
    # Extract MARC 001 mapping
    print("Extracting MARC 001 control numbers...")
    title_to_marc = extract_marc_mapping('data/RANDPUB_20250707.mrc')
    ...
    # Get all RAND publications with hash-based IDs
    cur.execute("""
        SELECT doctrove_paper_id, doctrove_title, doctrove_source_id 
        FROM doctrove_papers 
        WHERE doctrove_source = 'randpub' 
        AND doctrove_source_id LIKE 'RAND_PUBLICATION_%'
    """)
    
    records = cur.fetchall()
    print(f"Found {len(records)} RAND publications with hash-based IDs")
    ...
    print(f"Updated {updated_count} RAND publications with MARC 001 control numbers")
