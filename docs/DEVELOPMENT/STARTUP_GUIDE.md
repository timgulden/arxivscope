# DocScope/DocTrove Startup Guide
...
1. **Database** (PostgreSQL) - Stores papers and embeddings on port 5432
...
5. **Legacy Dash Frontend** - Legacy frontend on port 8050 (frozen, for reference only)
...
> **Note**: For historical multi-environment setup information, see [MULTI_ENVIRONMENT_SETUP.md](../DEPLOYMENT/MULTI_ENVIRONMENT_SETUP.md). The current system uses a single local environment configuration.
...
**Location:** `docscope-platform/services/docscope/react/` directory (if exists)  
**Port:** 3000  
**Execution Method:** npm dev server
...
Should return HTML content from the React application.

**Note**: The React frontend uses Vite and supports hot-reloading. Changes will automatically reload without restarting the server.
...
**Note**: The Dash frontend is frozen (bug fixes only). All new development should use the React frontend on port 3000.
...
**Solution:** Check database configuration. The system uses `.env.local` for configuration. Verify:
- PostgreSQL is running on port 5432 (internal drive)
- Database `doctrove` exists
- Trust authentication is configured (local setup)

Check `doctrove-api/config.py` for environment variable usage:
```python
...
### Issue 5: Frontend Port Confusion

**Problem:** Can't access frontend on expected port.

**Cause:** React frontend runs on port 3000, legacy Dash on port 8050.

**Solution:** Use port 3000 for React frontend (recommended):
```bash
# CORRECT (React)
http://localhost:3000
...
# Test frontend (React on port 3000)
...
3. **Terminal 3 - React Frontend:**
   ...
   Or for legacy Dash frontend (not recommended):
   ```bash
   python -m docscope.app
   ```
...
lsof -i :5001
lsof -i :3000
lsof -i :8050  # Legacy Dash frontend (if used)
```
...
| `Connection refused` | Wrong port | Use port 5001 for API, 3000 for React frontend |
| `Database connection failed` | Database not running | Start PostgreSQL on port 5432 (internal drive) |
...
- **Key Settings:** Database connection, API/Frontend ports, model paths
- **Location:** Project root directory
- **Template:** See `env.local.example` for configuration options

### API Configuration
- **File:** `doctrove-api/config.py`
- **Key Settings:** Database connection (reads from `.env.local`), batch sizing
- **Port:** 5001 (configured via environment variables)

### Frontend Configuration
- **React Frontend:** Configuration via `.env.local` (`NEW_UI_PORT=3000`)
- **Legacy Dash Frontend:** `docscope/config/settings.py` (frozen)

### Database Configuration
- **File:** `doctrove_schema.sql`
- **Purpose:** Database schema and functions
- **Location:** Internal drive (`/opt/homebrew/var/postgresql@14`)
