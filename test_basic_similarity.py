#!/usr/bin/env python3
"""
Test basic similarity queries to verify if they actually work.
Very simple test with minimal complexity.
"""
...
def test_simple_limit():
    ...
    return safe_database_operation(operation, timeout_seconds=5, test_name="Simple LIMIT")
...
def test_embedding_limit():
    ...
    return safe_database_operation(operation, timeout_seconds=5, test_name="Embedding LIMIT")

def test_basic_similarity():
    """Test 4: Basic similarity query - very simple."""
    ...
    return safe_database_operation(operation, timeout_seconds=10, test_name="Basic Similarity Query")

def test_similarity_with_similarity_score():
    """Test 5: Similarity query with similarity score calculation."""
    test_embedding = generate_test_embedding()
    ...
    return safe_database_operation(operation, timeout_seconds=10, test_name="Similarity with Score")

def main():
    """Test basic similarity queries."""
    logger.info("🔬 Testing Basic Similarity Queries")
    logger.info("🎯 Goal: Verify if basic similarity queries actually work")
    logger.info("⏰ Timeout: 5-10 seconds per test")
    logger.info("=" * 60)
    ...
    tests = [
        ...
        ("Similarity with Score", test_similarity_with_similarity_score),
    ]
    ...
    # Summary
    logger.info(f"\n{'='*60}")
    logger.info("BASIC SIMILARITY TEST SUMMARY")
    logger.info(f"{'='*60}")
    ...
    if 'Basic Similarity' in results and results['Basic Similarity']['success']:
        ...
        logger.error("❌ Basic similarity queries are indeed hanging")
